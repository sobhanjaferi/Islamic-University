...
const Article = () => {
  ...
  return (
    ...
    >
      <ButtonArticle
        className={`transition-all duration-500 ease-in-out ${observer ? "translate-y-0" : "-translate-y-10 opacity-0"}`}
      />

      <Observer delay={900} observer={observer}>
        <h1
      ...
      <Observer delay={1100} observer={observer}>
        <p className={`text-gray-300 max-w-145 md:text-lg`}>
          صندوق پژوهش و فناوری دانشگاه آزاد اسلامی با هدف توسعه زیست‌بوم نوآوری،
          تأمین مالی طرح‌های فناورانه و حمایت از کسب‌وکارهای دانش‌بنیان فعالیت
          می‌کند.
        </p>
      </Observer>

      <Observer delay={1300} observer={observer}>
        <section
